...
topics_db = redis.Redis(host='localhost', port=6379, db=2, decode_responses=True)
...
def get_all_topics():
    ...
    for key in topics_db.scan_iter("*"):
        #tweet = topics_db.lrange(key, 0, -1)
        #topic = json.loads(tweet)
        topics.append(key)
    return jsonify(topics)

##Afficher tous les tweets liés à un sujet
@app.route('/tweets/<topic>', methods=['GET'])
